...
const String _googleMapsApiKey = "AIzaSyCPsKJWDqJfyvkjT5hZ2XgQOGXEcBvzXT4";
final _googleApiHeaders = {"x-ios-bundle-identifier": _appleBundleId};

//Platform.environment['API_KEY']

const prodApiProxy = "https://europe-west2-flutterbasedotcom.cloudfunctions.net/FlutterbaseTaxiWebDemo";
...
List<LatLng>? createPolylinePointsFromDirections(dir.DirectionsResponse response) {
  if (response.isOkay) {
    final polylineRawList = decodePolyline(response.routes[0].overviewPolyline.points);
    List<LatLng> polylinePointList = polylineRawList.map((p) => LatLng(p[0].toDouble(), p[1].toDouble())).toList();
